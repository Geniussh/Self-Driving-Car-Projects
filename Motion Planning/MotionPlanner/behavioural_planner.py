...
# Author: Ryan De Iaco, Kelvin Shen
# Additional Comments: Carlos Wang, Kelvin Shen
...
class BehaviouralPlanner:
    ...
    def transition_state(self, waypoints, ego_state, closed_loop_speed):
        ...
        # state.
            ...
            if abs(closed_loop_speed) <= STOP_THRESHOLD:
                ...
                self._stop_count = 0  # reset stop count
    ...
    def get_goal_index(self, waypoints, ego_state, closest_len, closest_index):
        ...
        while wp_index < len(waypoints) - 1:
            arc_length += np.linalg.norm([waypoints[wp_index][0] - waypoints[wp_index+1][0], 
                                            waypoints[wp_index][1] - waypoints[wp_index+1][1]])
            if arc_length >= self._lookahead: 
                break
            wp_index += 1
...
def get_closest_index(waypoints, ego_state):
    ...
    for i in range(len(waypoints)):
        cur_wp = np.array(waypoints[i][:2])
        ego_pos = np.array(ego_state[:2])
        temp_len = np.linalg.norm(cur_wp - ego_pos)
        if temp_len < closest_len:
            closest_len = temp_len
            closest_index = i
